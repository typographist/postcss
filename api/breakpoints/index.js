...
export const between = (breakVal1, breakVal2) => {
  ...
  if (!parseFloat(breakVal1) && !parseFloat(breakVal2)) {
    ...
    result['firtBreakValue'] = convertToEm(
      getValidBreakpoint(breakpoint1.breakpoint),
    );
    result['lastBreakValue'] = convertToEm(
      getValidBreakpoint(breakpoint2.breakpoint) - 1,
    );
  } else {
